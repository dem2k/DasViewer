...
public class DasListController {
	...
	public DasListController(ListView<String> fxListView, TableView fxHeaderTable, DasViewerController dasViewerController) {
	...
	private void onItemSelected(ObservableValue<? extends String> observable, String old, String selectedItem) {
		if (selectedItem==null||selectedItem.length() != 26) {
			dasViewerController.getKzBox().update(null);
			return;
		}
		DasTeilfeld tf = new DasTeilfeld(selectedItem);
		dasViewerController.getKzBox().update(tf);
	}
